...
class NewShape;
class NewCell;
class NewPan : public QObject
{
    Q_OBJECT
public:
    explicit NewPan(NewPaint *pPaint,int nCenterX,int nCenterY);
    virtual ~NewPan();

public:
    Coord*             m_pCoord;
    NewCell*           m_pNewCell[NEW_NUM];
...
public:
    ...
    double              m_dRunRadius[NEW_NUM];      //单元的运行半径
    double              m_dRunRadian[NEW_NUM];      //单元运行弧度,读取文件度数°转化为弧度rad,相对于x轴正方向
    QPainterPath        m_pathInsertects[NEW_NUM];  //重叠区域
    ...
    bool                m_bDealRuned[NEW_NUM];     //单元是否已到达目标位置，不运动的单元默认为已到达目标位置
public:
    QPainter*  m_pPainter;          //传递绘画
    double	   m_dZoom;			    //缩放比
    int		   m_nRow;			    //单元实际总行数
    int        m_nComTimerID;       //一般运行定时器ID
    int        m_nPlanTimerID;      //计划运行定时器ID
    int        m_nLastTimerID;      //结果运行定时器ID
public:
    ...
    int        m_nRunMode;//运行类型，一般运行，处理运行，结果运行,暂停按钮时判断使用
...
public:
    ...
    void    RunCellValue();            //一般直接运行时设置单元中心轴偏心值
    ...
    void    DealCenWave(int nID, int nPart);        //碰撞时处理波形，使波形变成2段式，最终最多3段式波形
    void    DealEccWave(int nID, int nPart);        //碰撞时处理波形，使波形变成2段式，最终最多3段式波形
    void    ResetCell(int nID,bool bStatus=STOP_STATUS,double dRadius=STOP_RADIUS,
                      double dRadian=STOP_RADIAN);//重新设置单元
public:
...
signals:
    void SendCollision(int nID,int mID);
